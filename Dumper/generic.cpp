...
uint8* FNamePool::GetEntry(FNameEntryHandle handle) const
{
    ...
    return (uint8*)(Blocks[handle.Block] + offsets.Stride * static_cast<uint64>(handle.Offset));
}
...
void FNamePool::DumpBlock(uint32 blockId, uint32 blockSize, std::function<void(std::string_view, uint32)> callback) const
{
    ...
    FNameEntryHandle entryHandle = { blockId, NULL };
    while (it < end)
    {
        UE_FNameEntry entry = UE_FNameEntry(it);
        auto [wide, len] = entry.Info();
        if (len)
        {
            std::string buf{}; //char buf[1024];
            auto wlen = entry.String(buf, wide, len); // entry.String(buf, wide, len);
            callback(std::string_view(buf.c_str(), (wlen ? wlen : len)), entryHandle); //callback(std::string_view(buf, len), entryHandle);
...
uint8* TUObjectArray::GetObjectPtr(uint32 id) const
{
    ...
    unsigned char* item = Read<uint8*>(chunk + withinChunkIndex);
